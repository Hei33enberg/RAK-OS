...
| `rak_research_extract` | any URL → clean markdown (SSRF-safe) | anon/free (RAK-billed), then paid |
| `rak_research_deep` | autonomous deep research with a URL bibliography (Tavily + synthesis) | paid |
...
## Tier / cost
`rak_research_web` and `rak_research_extract` are available anonymously/free with a hard 5/day/IP limit (RAK-billed centrally), then `paid`. `deep`, `fact_pack`, `summarize` are `paid` (credits per RAK pricing).
